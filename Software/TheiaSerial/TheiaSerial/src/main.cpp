...
TheiaSerial theiaSerial(D4, D1, 1);
byte payload[2] = {0x01, 0x02};

void setup(){}

void loop(){
  theiaSerial.send(payload, 2);
  payload[0] = theiaSerial.read(5000).id;
...
// // // Create a SoftwareSerial object on pins 10 (RX) and 11 (TX)
// // SoftwareSerial mySerial(D0, D1); 

// // void setup() {
// //   Serial.begin(9600);         // Start hardware serial (USB to computer)
...
// //     Serial.write(mySerial.read()); // Read from software serial, write to hardware serial
// //   }

// //   if (Serial.available()) {
// //     mySerial.write(Serial.read()); // Read from hardware serial, write to software serial
// //   }
// // }
